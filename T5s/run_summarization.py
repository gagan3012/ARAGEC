...
from m2scorer.scripts.m2scorer import evaluate as m2scorer_evaluate
...
class DataTrainingArguments:
    ...
    test_file_2015: Optional[str] = field(
        default=None,
        metadata={
            "help": "An optional input test data file to evaluate the metrics (m2) on (a jsonlines or csv file)."
        },
    )
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        if data_args.test_file_2015 is not None:
            data_files["test_2015"] = data_args.test_file_2015
            extension = data_args.test_file_2015.split(".")[-1]
        raw_datasets = load_dataset(
            extension,
    ...
    if training_args.do_predict:
        ...
        if data_args.test_file_2015 is not None:
            max_target_length = data_args.val_max_target_length
            predict_dataset_2015 = raw_datasets["test_2015"]
            if data_args.max_predict_samples is not None:
                max_predict_samples = min(
                    len(predict_dataset), data_args.max_predict_samples)
                predict_dataset = predict_dataset.select(
                    range(max_predict_samples))
            with training_args.main_process_first(desc="prediction dataset map pre-processing"):
                predict_dataset_2015 = predict_dataset_2015.map(
                    preprocess_function,
                    batched=True,
                    num_proc=data_args.preprocessing_num_workers,
                    remove_columns=column_names,
    ...
    def compute_metrics(eval_preds):
        ...
        m2 = "../data/QALB-0.9.1-Dec03-2021-SharedTasks/data/2014/test/QALB-2014-L1-dev.m2"
        p,r,f1,f5 = m2scorer_evaluate(decoded_preds,m2,verbose=False)
        result["precision"] = p
        result["recall"] = r
        result["f1"] = f1
        result["f0.5"] = f5
    ...
    trainer = Seq2SeqTrainer(
        ...
        compute_metrics=compute_metrics if not training_args.predict_with_generate else None,
    )
    ...
    if training_args.do_predict:
        ...
        metrics["predict_samples"] = min(
            max_predict_samples, len(eval_dataset))
        ...
        if trainer.is_world_process_zero():
            if training_args.predict_with_generate:
                ...
                output_prediction_file = os.path.join(
                    training_args.output_dir, "generated_predictions_dev.txt")
        ...
        logger.info("*** Predict 2015 ***")

        predict_results = trainer.predict(
            predict_dataset_2015, metric_key_prefix="predict")
        metrics = predict_results.metrics
        max_predict_samples = (
            data_args.max_predict_samples if data_args.max_predict_samples is not None else len(
                predict_dataset)
        ...
        if trainer.is_world_process_zero():
            if training_args.predict_with_generate:
                ...
                predictions = [pred.strip() for pred in predictions]
                output_prediction_file = os.path.join(
                    training_args.output_dir, "generated_predictions_2015.txt")
                with open(output_prediction_file, "w") as writer:
                    writer.write("\n".join(predictions))
